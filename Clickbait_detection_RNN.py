"""
MODEL ACCURACY'NİN DÜŞÜK OLMASI VE TAHMİNLERİN YANILTICI OLMASI RNN MODELİNİN
UZUN SÜRELİ BAĞIMLILIKLARI ÖĞRENEMEMESİNDEN AYRICA TEK YÖNLÜ OLMASINDAN KAYNAKLIDIR.
RNN İLE LSTM ARASINDAKİ FARKI ANLAMAK İÇİN BU MODEL LSTM'İN ALTERNATİFİ OLARAK GELİŞTİRİLMİŞTİR.
"""
...
from tensorflow.keras.layers import Embedding, SimpleRNN, Dense, SpatialDropout1D, Bidirectional
...
def clean_text(text):
    ...
    final_tokens = []
    for t in tokens:
        if t not in turkish_stopwords or t in ['!', '?']:
            if t in ['!', '?']:
